...
-- Automatic
--=========================================
-- 🔥 HYPER-X AUTO PARRY V2.3 (INTEGRATED)
--=========================================
local Config = {
    Enabled = false,
    Distance = 9,
...
-- // [ GUI CONSTANTS - KEEP SAME AS YOURS ]
...
-- // [ REBUILD UI EXACTLY LIKE ORIGINAL ]
...
local TopBar = Instance.new("Frame", Main) TopBar.Size = UDim2.new(1, -2, 0, 26) TopBar.Position = UDim2.fromOffset(1, 1) TopBar.BackgroundColor3 = Colors.Background2 TopBar.BorderSizePixel = 0 ApplyStyle(TopBar, 11, Colors.Red, 1, 0.8)

local Header = Instance.new("TextLabel", TopBar) Header.Size = UDim2.new(1, -60, 1, 0) Header.Position = UDim2.fromOffset(55, 0) Header.BackgroundTransparency = 1 Header.Text = "REAPER X SYSTEM" Header.TextColor3 = Colors.White Header.TextSize = 10 Header.Font = Enum.Font.GothamBold Header.TextXAlignment = Enum.TextXAlignment.Left
    ...
    State.Connections[char] = animator.AnimationPlayed:Connect(function(track)
        ...
        if ATTACK_ANIMS[track.Animation.AnimationId:match("%d+")] then
            ...
            if myChar and myChar:GetAttribute("State") ~= "Downed" then
                local maxRange = Config.Distance
                if (myChar.PrimaryPart.Position - char.PrimaryPart.Position).Magnitude <= maxRange then
                    if IsThreatening(char, myChar, maxRange) then -- New V2.3 Check
                        State.Cooldown = true
...
-- // [ RENDER LOOP ]
local RangeAdorn = Instance.new("CylinderHandleAdornment", workspace.Terrain) RangeAdorn.Height = 0.1 RangeAdorn.Transparency = 0.5
    ...
    if myChar and myChar.PrimaryPart then
        ...
        if Config.ShowStatusUI then
            ...
            if State.CurrentCD > 0 then
                ...
                StatusLabel.TextColor3 = Colors.TrafficGreen Indicator.BackgroundColor3 = Colors.TrafficGreen
            end
        ...
        if Config.ShowCircle and myRole == "Survivors" then
            ...
            local currentRange = Config.Distance
            RangeAdorn.Color3 = (State.CurrentCD > 0 and Colors.TrafficYellow) or (closestDist <= currentRange and Colors.TrafficRed) or Colors.TrafficGreen
            RangeAdorn.Radius = currentRange
            RangeAdorn.InnerRadius = currentRange - 0.2
